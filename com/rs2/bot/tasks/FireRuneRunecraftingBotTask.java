...
public final class FireRuneRunecraftingBotTask
extends BotTaskDefinition {
    private static Position aa = new Position(3269, 3167, 0);
    private static BotRoute[] ab = new BotRoute[]{new BotRoute(new Position[]{new Position(3273, 3167, 0), new Position(3280, 3179, 0), new Position(3291, 3184, 0), new Position(3296, 3196, 0), new Position(3301, 3202, 0), new Position(3301, 3222, 0), new Position(3301, 3237, 0), new Position(3308, 3248, 0)}), new BotRoute(new Position[]{new Position(2579, 4843, 0)})};

    public FireRuneRunecraftingBotTask(int n) {
        super(aa, ab, 0, false, 1);
    }

    @Override
    public final boolean meetsUnlockRequirements(Player player) {
        return player.getSkillManager().getCurrentLevels()[20] >= 14;
    }

    @Override
    public final boolean isWithinProgressionRange(Player player) {
        return player.getSkillManager().getCurrentLevels()[20] < 20 || !player.ownsItem(5533) && !player.ownsItem(1446);
    }

    @Override
    public final ArrayList getRequiredItems(Player player) {
        ArrayList<ItemStack> arrayList = new ArrayList<ItemStack>();
        if (player.ownsItem(5537) && ItemDefinition.isDefined(5537)) {
            arrayList.add(new ItemStack(5537, 1));
        } else {
            arrayList.add(new ItemStack(1442, 1));
        }
    ...
    public final void configureTaskInteractionTargets(Player player) {
        ...
        player.botInteractionTargetIds.add(2482);
    }
    ...
    public final void prepareTaskInventory(Player player) {
        ...
        ItemStack[] itemStackArray = new ItemStack[]{new ItemStack(5537), new ItemStack(1436, 28)};
        if (GameUtil.randomInt(2) == 0 || !ItemDefinition.isDefined(5537)) {
            itemStackArray = new ItemStack[]{new ItemStack(1442), new ItemStack(1436, 27)};
        }
        player.botTaskRequiredItems = itemStackArray;
        if (itemStackArray[0].getId() == 5537) {
    ...
    public final void prepareTaskCombatLoadout(Player player) {
        ...
        if ((n = n - n / 5 + GameUtil.randomInt(n2)) < 14) {
            n = 14;
        }
