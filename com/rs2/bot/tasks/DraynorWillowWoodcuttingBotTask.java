...
public final class DraynorWillowWoodcuttingBotTask
extends BotTaskDefinition {
    ...
    public DraynorWillowWoodcuttingBotTask(int n) {
        super(aa, ab, 0, false, 4);
    }

    @Override
    public final boolean meetsUnlockRequirements(Player player) {
        if (player.getSkillManager().getCurrentLevels()[8] < 30) {
            return false;
        }
    ...
    public final void configureTaskInteractionTargets(Player player) {
        ...
        int[] nArray = TreeDefinition.collectObjectIds(new TreeDefinition[]{TreeDefinition.WILLOW});
        int n = nArray.length;
        int n2 = 0;
        while (n2 < n) {
            int n3 = nArray[n2];
